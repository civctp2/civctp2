...
game, then read the next section, \ref{sec:pre-gameSetup}.
...
into learning how to play at section \ref{sec:turns}.

\section{Pre-game Setup}
\label{sec:pre-gameSetup}

(Various start-up options explained.)
After the intro movie has played you will find yourself in the Apolyton Edition
start menu.

From here you can select the type of game you wish to play and various in game
options.
...
\item[Email game] will allow you to play by email.
...
\item Save Game(S)
\item Load Game(L)
\item Restart (with the same game settings) (Ctrl+z)
...
NOTE: It takes a few(x?) turns for your War Status to change to the new
...
Intelligence --- provides more detail on
...
\item Select an additional Request, Offer or Treaty to make an exchange with
...
comparison to the other AI civilisations.
...
this goal will have an asterix(*) symbol next to them, so you can see what
...
selected a city from the list in the `Status'screen you will be able to quickly
...
buttons, they work the same here as described previously.
