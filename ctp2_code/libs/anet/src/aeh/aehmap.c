...
static void aeh_map_addFunc(aeh_map_t *aehmap, char *name, uint32 address)
{
...
static void aeh_map_Unload(aeh_map_t *aehmap, sint32 bWriteAlg);
...
static sint32 aeh_map_algRead(aeh_map_t *aehmap, char *algpath) {
	...
	if ((fread(&tag, sizeof(uint16), 1, fh) != 1) ||
		(tag != aeh_map_func_alg_MAGIC) ||
		(fread(&prefadr, sizeof(uint32), 1, fh) != 1)) {
	...
	while ((nread = fread(ptr, 1, nsize, fh)) > 0) {
		...
		ptr = buffer;
		while (ptr + (sizeof(uint32)+sizeof(uint16)) < eobuf){
			uint16 namelen;
			address = *((uint32*)ptr);
			namelen = *((uint16*)(ptr + sizeof(uint32)));
			if (ptr+(sizeof(uint32)+sizeof(uint16)+namelen) < eobuf) {
...
static sint32 aeh_map_algWrite(aeh_map_t *aehmap, char *algpath) {
	FILE *fh;
	sint32 bWriteError = 0;
	aeh_map_func_t *curr = aehmap->firstfunc;
	char buffer[BUFFER_SIZE], *ptr;
	uint16 tag = aeh_map_func_alg_MAGIC;
	fh = fopen(algpath, "rb");
	...
	fh = fopen(algpath, "wb");
	if (!fh || (fwrite(&tag, sizeof(uint16), 1, fh) != 1) ||
		(fwrite(&(aehmap->load_addr), sizeof(uint32), 1, fh) != 1)) {
	...
	if (!bWriteError) {
		...
		if (!bWriteError && (fwrite(&tag, sizeof(uint16), 1, fh) != 1))
			bWriteError = 1;
	}
	fclose(fh);
	if (bWriteError) {
...
static sint32 aeh_map_Load(aeh_map_t *aehmap)
{
	FILE *fp;
...
sint32 aeh_map_Find(aeh_mapcat_t *aehmapcat, aeh_modfile_t *aehmodfile)
{
	...
	for (i = 0; i < aehmapcat->nmap; i++) {
		...
		if (mapbuf->crc == aehmodfile->crc) {
			if (!mapbuf->firstfunc) {
				sint32 err = aeh_map_Load(mapbuf);
				if (err != aeh_RES_OK) {
