...
void *assoctab_subscript_insert(assoctab_t *tab, int key, int n)
{
	...
	pi = (assoctab_item_t *) dynatab_subscript_insert((dynatab_t *)tab, n);
	if (!pi)
		return NULL;
	//assert(pi[0..tab->unit-1] == 0);
	pi->key = key;
...
void test1(assoctab_t *pt, int callnum)
{
	...
	/* Insert a wierd value at offset 10 */
	{
		p = assoctab_subscript_insert(pt, 1000, 10);
		if (p == NULL) {
			printf("insert: test failed\n");
			exit(1);
		}
		if (*p != 0) {
			printf("insert initial value test: test failed\n");
			errs++;
		}
		*p = 1000;
	}
	...
	if (pt->n_used != 1) {
		printf("delete2.5: test failed.  Expected n_used = 1, got %d\n", pt->n_used);
		errs++;
	}
	if (assoctab_subscript_delete(pt, 1000)) {
		printf("delete2.5: delete failed.\n");
		errs ++;
	}
	if (pt->n_used != 0) {
