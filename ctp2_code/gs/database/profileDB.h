...
public:
	...
	void SetLeaderName(const MBCHAR *name)		{ strcpy(m_leaderName, name); }
	void SetCivName(const MBCHAR *name)		{ strcpy(m_civName, name); }
	void SetSaveNote(const MBCHAR *note)		{ strcpy(m_saveNote, note); }
	void SetIsSaved(BOOL isSaved)			{ m_isSaved = isSaved; }
	void SetGameName(const MBCHAR *name)		{ strcpy(m_gameName, name); }
	...
	void SetRiskLevel(uint32 x)			{ Assert(x<5); m_risklevel = x; }
	...
	void SetGameWatchDirectory(const MBCHAR *dir)	{ strcpy(m_gameWatchDirectory, dir); }
	...
	const MBCHAR	*GetLeaderName() const		{ return m_leaderName; }
	const MBCHAR	*GetCivName() const		{ return m_civName; }
	const MBCHAR	*GetSaveNote() const		{ return m_saveNote; }
	BOOL		IsSaved() const			{ return m_isSaved; }
	const MBCHAR	*GetGameName() const		{ return m_gameName; }
