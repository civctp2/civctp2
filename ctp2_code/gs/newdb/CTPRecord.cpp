...
bool CTPRecord::ParseStringIdInArray(DBLexer *lex, sint32 **array, char ***sarray, sint32 *numElements)
{
	...
	do
	{
		...
		if(*numElements > 0) {
			...
			*array = new sint32[(*numElements) + 1];
			...
			char **oldSArray = *sarray;
			*sarray = new char *[*numElements + 1];
			memcpy(*sarray, oldSArray, (*numElements) * sizeof(char *));
			delete [] oldSArray;
		}
		else
		{
		...
		(*sarray)[*numElements] = new char[strlen(value) + 1];
		strcpy((*sarray)[*numElements], value);
		*numElements += 1;
	}
...
bool CTPRecord::ParseStringIdInArray(DBLexer *lex, sint32 *array, char **sarray, sint32 *numElements, sint32 maxSize)
{
	if(lex->PeekAhead() != k_Token_Name)
	{
