...
int main(int argc, const char **argv)
{
	...
	if(!outputDir) {
		fprintf_s(stderr, "Usage: %s [-r] [outputdir | -i inputfile]\n", argv[0]);
		exit(errorFound);
	}
...
void db_set_output_dir(const char *dir)
{
	...
	strncpy_s(s_output_dir, dir, PATH_MAX);
}
...
FILE *db_open_file(const char *filename)
{
	...
	fprintf_s(outfile, "\n/*\n");
	fprintf_s(outfile, " * DO NOT EDIT THIS FILE!\n");
	fprintf_s(outfile, " * It is generated automatically by ctpdb\n");
	fprintf_s(outfile, " */\n");
...
void db_maybe_copy(char * newFilePath)
{
	...
	strcpy_s(oldFilePath, newFilePath);
	...
	*dot = 0;
    ...
    if (hasOldFile)
    {
	    ...
	    if (hasChanges)
	    {
			...
			sprintf_s(backupPath, "%s.old", oldFilePath);
...
void db_end_record(char *name)
{
	...
	sprintf_s(filename, "%s" FILE_SEP "%sRecord.h.new",
	...
	sprintf_s(filename, "%s" FILE_SEP "%sRecord.cpp.new",
	        db_get_code_directory(), name);
	...
	sprintf_s(filename, "%s" FILE_SEP "%sRecord.stamp",
	        db_get_code_directory(), name);
	FILE * stamp = fopen(filename, "w");
	Assert(stamp);
	if(stamp) {
		fprintf_s(stamp, "//%" PRId64 "\n", time(0));
		fclose(stamp);
	}
