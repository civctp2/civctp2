...
// - Add the Target Position (X,Y) to The ArmyText (the text that is
//   shown on the screen in Debug mode, with the command 'armytext'
//
//----------------------------------------------------------------------------
...
void CTPGoal::Compute_Needed_Troop_Flow()
{
    ...
    { }
    else { m_current_needed_strength.Set_Pos_Strength(Get_Target_Pos()); }
...
const Path & found_path)
{
    ...
    if (sub_task == SUB_TASK_GOAL)
    {
        ...
        else { test = first_army->Get_Army()->TestOrderHere(order_rec, dest_pos); }
    }
    else { order_rec = CtpAi::GetMoveOrder(); }
    ...
    test == ORDER_TEST_NO_MOVEMENT)
    {
        ...
        MBCHAR * myString = new MBCHAR[strlen(myText) + 20];
        memset(myString, 0, strlen(myText) + 20);
        for (uint8 myComp = 0; myComp < strlen(myText) - 5; myComp++)
        { myString[myComp] = myText[myComp + 5]; }
        sprintf(myString, "%s (%d,%d)", myString, my_target_loc.x, my_target_loc.y);
...
bool CTPGoal::GotoTransportTaskSolution(CTPAgent_ptr the_army, CTPAgent_ptr the_transport, const SUB_TASK_TYPE & sub_task)
{
    ...
    switch (sub_task)
    {
        ...
        case SUB_TASK_CARGO_TO_BOARD:
            ...
            else { transport_at_rendezvous = true; }
...
bool CTPGoal::GotoGoalTaskSolution(CTPAgent_ptr the_army, const MapPoint & goal_pos, const SUB_TASK_TYPE & sub_task)
{
    ...
    else { move_success = TryTransport(the_army, goal_pos); }
