...
AllinoneWindow::~AllinoneWindow()
{
	if ( g_rulesWindow )
	{
		delete g_rulesWindow;
		g_rulesWindow = NULL;
	}

	if ( g_exclusionsWindow )
	{
		delete g_exclusionsWindow;
		g_exclusionsWindow = NULL;
	}
	...
	else
	{
		if ( m_mode == CONTINUE_CREATE || m_mode == CONTINUE_JOIN )
		{
			...
			for ( sint32 i = 2; i < g_nsTribes->GetNumTribes(); i++ )
			{
				...
				BOOL success =
#endif
				WhoHasTribe(i, &curKey, &curIsAI, &curIsFemale);
				Assert( success );
			...
			for(sint32 i = 0; i < k_MAX_PLAYERS; i++) {
				if(m_scenInfo.m_legalCivs[i] > 0) {
					...
					BOOL success =
#endif
					WhoHasTribe(m_scenInfo.m_legalCivs[i] + 1,
							&curKey, &curIsAI,
							&curIsFemale);
					Assert(success);
					if(curKey == 0) {
	...
	BOOL unassign )
{
	...
	BOOL success =
#endif
	WhoHasTribe(index, &curKey, &curIsAI, &curIsFemale);
	Assert( success );
...
AUI_ERRCODE AllinoneWindow::Idle( void )
{
	...
	if(joinedgame && g_netfunc->GetStatus() == NETFunc::OK) {
		...
		if ( g_nsUnits ) delete g_nsUnits;
		g_nsUnits = new ns_Units;
		if ( g_nsImprovements ) delete g_nsImprovements;
		g_nsImprovements = new ns_Improvements;
		if ( g_nsWonders ) delete g_nsWonders;
...
AUI_ERRCODE AllinoneWindow::SetParent( aui_Region *region )
{
	...
	AUI_ERRCODE errcode = ns_Window::SetParent( region );
...
void AllinoneWindow::UpdatePlayerButtons( void )
{
	...
	if ( item )
	{
		...
		else
		{
			...
			if ( item->GetPlayer()->IsMe() )
			{
				m_controls[ CONTROL_PPTSWITCH ]->Enable( false );
			}
			else
			{
				m_controls[ CONTROL_PPTSWITCH ]->Enable( true );
			}
		}
	...
	uint32 data )
{
	...
	char join = ((aui_Switch *)control)->GetState();

	g_gamesetup.SetDynamicJoin(join);
	w->UpdateGameSetup();
}
...
{
	if (action != (uint32)AUI_SWITCH_ACTION_ON &&
	    action != (uint32)AUI_SWITCH_ACTION_OFF)
		return;
	...
	void* cookie )
{
	...
	if ( item->IsAI() )
	{
		if ( g_netfunc->IsHost() )
		{
			...
			BOOL success =
#endif
			w->WhoHasTribe(index, &curKey, &curIsAI, &curIsFemale);
			Assert( success );
