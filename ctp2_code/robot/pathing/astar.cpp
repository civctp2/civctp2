...
for  (tmp.x=0; tmp.x<size->x; tmp.x++) { 
    ...
    do {  
		...
		if (IsInAvoidList(next_pos,avoidList)) continue;
...
BOOL Astar::IsInAvoidList(const MapPoint & pos, const MapPoint_List & avoidList)
{

    MapPoint_List::const_iterator avoid_iter;
    for (avoid_iter = avoidList.begin(); avoid_iter != avoidList.end(); avoid_iter++)	
	{
		if ((*avoid_iter) == pos)
		{
			return true;
